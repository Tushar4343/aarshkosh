...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.aarshchapter.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.aarshchapters.update", [$aarshchapter->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('granth_title') ? 'is-invalid' : '' }}" name="granth_title_id" id="granth_title_id" required>
                    @foreach($granth_titles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('granth_title_id') ? old('granth_title_id') : $aarshchapter->granth_title->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('arshchapter_no') ? 'is-invalid' : '' }}" type="number" name="arshchapter_no" id="arshchapter_no" value="{{ old('arshchapter_no', $aarshchapter->arshchapter_no) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('arshchapter_title') ? 'is-invalid' : '' }}" type="text" name="arshchapter_title" id="arshchapter_title" value="{{ old('arshchapter_title', $aarshchapter->arshchapter_title) }}" required>
