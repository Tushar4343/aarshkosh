...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.aarshgranth.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.aarshgranths.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('aarsh_book') ? 'is-invalid' : '' }}" name="aarsh_book_id" id="aarsh_book_id" required>
                    @foreach($aarsh_books as $id => $entry)
                        <option value="{{ $id }}" {{ old('aarsh_book_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('arshbook_no') ? 'is-invalid' : '' }}" type="number" name="arshbook_no" id="arshbook_no" value="{{ old('arshbook_no', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('arshbook_title') ? 'is-invalid' : '' }}" type="text" name="arshbook_title" id="arshbook_title" value="{{ old('arshbook_title', '') }}" required>
