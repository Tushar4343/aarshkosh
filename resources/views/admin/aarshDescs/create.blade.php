...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.aarshDesc.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.aarsh-descs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chapter_title') ? 'is-invalid' : '' }}" name="chapter_title_id" id="chapter_title_id" required>
                    @foreach($chapter_titles as $id => $entry)
                        <option value="{{ $id }}" {{ old('chapter_title_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
